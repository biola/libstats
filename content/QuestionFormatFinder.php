...
class QuestionFormatFinder extends Finder {
  ...
  function getAllQuestionFormats() {
    $query =
    "SELECT
        question_formats.question_format_id,
        question_formats.question_format,
        library_question_formats.library_id
    FROM
        question_formats
    ...
    ORDER BY library_question_formats.list_order";
  ...
  function getQuestionFormat($question_format_id){
		$query =
		"SELECT
			question_format
		FROM
		...
		$result = $this->db->getOne($query, array($question_format_id));
		return $result;
	}
